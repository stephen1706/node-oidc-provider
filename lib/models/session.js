...
module.exports = function getSession(provider) {
  ...
  class Session {
    ...
    acceptedScopesFor(clientId, scopes) {
      const authorization = authorizationFor.call(this, clientId);

      if (scopes) {
        authorization.acceptedScopes = scopes;
        return undefined;
      }

      return authorization.acceptedScopes || [];
    }
    ...
    rejectedClaimsFor(clientId, claims) {
      ...
      if (claims) {
        authorization.rejectedClaims = claims;
        return undefined;
      }

      return authorization.rejectedClaims;
    }
